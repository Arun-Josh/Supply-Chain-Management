...
class EditConsumers extends React.Component {
    state = {
        _id: new URLSearchParams(this.props.location.search).get("id"),
        isConsumerEdited: false,
        options: {
    ...
    componentDidMount() {
        Axios.get(`${CONSTANTS.SERVER_URL}/api/consumers/` + this.state._id).then(response => {
            this.setState({
    ...
    onSubmit = (e) => {
        ...
        Axios.post(`${CONSTANTS.SERVER_URL}/api/consumers/update/` + this.state._id, addconsumer)
            .then(setTimeout(() => (this.setState({ isConsumerEdited: true })), 1000)).then(res => {
                this.props.enqueueSnackbar(
    ...
    render() {
        return (
            <>
                ...
                {this.state.isConsumerEdited ? <Redirect to="/admin/consumers" /> : null}
            </>
        );
    }
}
...
export default withSnackbar(EditConsumers);