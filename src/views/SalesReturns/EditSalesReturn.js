...
class AddSalesReturn extends React.Component {

    state = {
        _id: new URLSearchParams(this.props.location.search).get("id"),
        sales_return_for: "",            // supplier id
        commodity_group: "",            // id
        date_of_sales_return: new Date(),
        sales_invoice_no: "",
        total: 0,
        metric_ton: "",
    ...
    handleSubmit = e => {
        ...
        payload.sales_return_for = this.state.sales_return_for;
        ...
        payload.date_of_sales_return = this.state.date_of_sales_return;
        payload.sales_invoice_no = this.state.sales_invoice_no;
        ...
        if (payload.sales_return_for === "") {
            this.props.enqueueSnackbar("Sales Return for is mandatory !", {
                variant: "error"
            });
            return;
        }
        ...
        payload.sales_return_description = this.state.sales_return_description;

        //Disable submit button
        this.setState({ isSubmitBtnDisabled: true })

        Axios.post(`${CONSTANTS.SERVER_URL}/api/sales-returns/update/` + this.state._id, payload)
            .then(res => {
                console.log(res);
                this.props.enqueueSnackbar(
                    `Sales return edited successfully`,
                    {
                        variant: "success"
                    }
                );
    ...
    handleDateChange = e => {
        ...
        if (e.isValid) {
            const DATE = e.format();
            this.setState({ date_of_sales_return: DATE }, console.log(this.state));
    ...
    componentDidMount = () => {
        ...
        Axios.get(`${CONSTANTS.SERVER_URL}/api/consumers/`).then(res => {
            if (res.data.length !== 0) {
                ...
                newOptions.sales_return_for = consumers;
        ...
        Axios.get(`${CONSTANTS.SERVER_URL}/api/sales-returns/` + this.state._id).then(response => {

            this.setState({
                sales_return_for: response.data.sales_return_for,
                commodity_group: response.data.commodity_group,
                date_of_sales_return: new Date(response.data.date_of_sales_return),
                sales_invoice_no: response.data.sales_invoice_no,
                ...
                sales_return_description: response.data.sales_return_description,
    ...
    render() {
        return (
            <>
                            ...
                            <Card className="bg-secondary shadow">
                                <CardHeader className="bg-white border-0">
                                    <Row className="align-items-center">
                                        <Col xs="8">
                                            <h3 className="mb-0">Sales Return</h3>
                                        </Col>
                                    </Row>
                                </CardHeader>
                                <CardBody>
                                            ...
                                            <Row>
                                                <Col lg="12">
                                                    <FormGroup>
                                                        ...
                                                        <br />
                                                        <Select
                                                            name="sales_return_for"
                                                            placeholder="Sales Return for..."
                                                            options={this.state.options.sales_return_for}
                                                            value={{
                                                                value: this.state.sales_return_for,
                                                                label: this.getOptionLabel(this.state.sales_return_for, "sales_return_for", "value", "label"),
                                                            }}
                                                    ...
                                                    <FormGroup>
                                                        ...
                                                        <FormGroup>
                                                            <InputGroup className="input-group-alternative">
                                                                ...
                                                                <ReactDatetime
                                                                    inputProps={{
                                                                        placeholder: "Sales Return date",
                                                                        disabled: true
                                                                    }}
                                                                    utc={true}
                                                                    value={this.state.date_of_sales_return}
                                    ...
                                    <div className="pl-lg-4">
                                        <FormGroup>
                                            <Input
                                                ...
                                                type="textarea"
                                                name="sales_return_description"
                                                value={this.state.sales_return_description}
                                                onChange={this.handleChange}
                                            />
                                        </FormGroup>
                                    </div>
                ...
                {/* Redirect on submit */}
                {this.state.isSalesSuccess ? <Redirect to="/admin/sales-return" /> : null}
...
export default withSnackbar(AddSalesReturn);
