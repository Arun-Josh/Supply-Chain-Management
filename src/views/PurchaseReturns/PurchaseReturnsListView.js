...
class PurchaseReturnListView extends React.Component {
    ...
    generateTableData = (rawData) => {
        ...
        rawData.forEach((row) => {
            ...
            let newRow = [row.date_of_purchase_return, row.users[0]?row.users[0].user_name:"", row.sub_commodities, row.invoice_no, row.net_total, <Button className="btn-icon btn-2" color="primary" size="sm" type="button" onClick={() => { window.location.href = "/admin/purchase-return/edit?id=" + row._id }}>
    ...
    render() {
            ...
            },
            {
                name: "Particulars", label: "Particulars", options: {
                    filter: false, sort: false, customBodyRender: (value, tableMeta, updateValue) => {
                        ...
                        return (
                            <>
                                <div>{value[i].sub_commodity_name}&nbsp;{value[i].sub_commodity_quantity}</div>
        ...
        return (
            <>
                <CommonHeader buttonText="Add Purchase Return" redirectURL={`/admin/purchase-return/add`} />

                <DateSearch getUsersLink={`${CONSTANTS.SERVER_URL}/api/suppliers/`} submitLink={`${CONSTANTS.SERVER_URL}/api/purchase-returns/`} columns={columns} generateTableData={this.generateTableData} options={options} isSubmitBtnDisabled={this.state.isSubmitBtnDisabled} disableSubmitButton={this.disableSubmitButton} />
            </>
        )
    }
...
export default withSnackbar(withStyles(customStyles, { name: "PurchaseReturnListView.js" })(PurchaseReturnListView));
