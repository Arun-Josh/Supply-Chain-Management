...
class SalesListView extends React.Component {
    ...
    generateTableData = (rawData) => {
        ...
        rawData.forEach((row) => {
            ...
            let newRow = [row.date_of_sales, row.users[0].user_name, row.sub_commodities, row.invoice_no, row.net_total, <Button className="btn-icon btn-2" color="primary" size="sm" type="button" onClick={() => { window.location.href = "/admin/sales/edit?id=" + row._id }}>
    ...
    render() {
        ...
        return (
            <>
                <CommonHeader buttonText="Add Sales" redirectURL={`/admin/sales/add`} />

                <DateSearch getUsersLink={`${CONSTANTS.SERVER_URL}/api/consumers/`} submitLink={`${CONSTANTS.SERVER_URL}/api/sales/`} columns={columns} generateTableData={this.generateTableData} options={options} isSubmitBtnDisabled={this.state.isSubmitBtnDisabled} disableSubmitButton={this.disableSubmitButton} />
            </>
        )
    }
...
export default withSnackbar(withStyles(customStyles, { name: "SalesListView.js" })(SalesListView));
