...
class IncomesListView extends React.Component {
    ...
    generateTableData = (rawData) => {
        ...
        rawData.forEach((row) => {
            ...
            let newRow = [row.income_type, row.date_of_income, row.commodity_groups[0]?row.commodity_groups[0].commodity_group_name:"", row.invoice_no, row.amount, <Button className="btn-icon btn-2" color="primary" size="sm" type="button" onClick={() => { window.location.href = "/admin/incomes/edit?id=" + row._id }}>
    ...
    render() {
        ...
        return (
            <>
                <CommonHeader buttonText="Add Income" redirectURL={`/admin/incomes/add`} />

                <DateSearch getUsersLink={`${CONSTANTS.SERVER_URL}/api/suppliers/`} submitLink={`${CONSTANTS.SERVER_URL}/api/incomes/`} columns={columns} generateTableData={this.generateTableData} options={options} isSubmitBtnDisabled={this.state.isSubmitBtnDisabled} disableSubmitButton={this.disableSubmitButton} />
            </>
        )
    }
...
export default withSnackbar(withStyles(customStyles, { name: "IncomesListView.js" })(IncomesListView));
