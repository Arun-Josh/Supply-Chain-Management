...
class SuppliersListView extends React.Component {
    ...
    getData() {
        ...
        Axios.get(`${CONSTANTS.SERVER_URL}/api/suppliers/`, {
    ...
    generateTableData = (rawData) => {
        ...
        rawData.forEach((row) => {
            ...
            let newRow = [row.user_name, row.address + "*" + row.district, row.phone, row.email_id, row.account_no, row.balance, <Button className="btn-icon btn-2" color="primary" size="sm" type="button" onClick={() => { window.location.href = "/admin/suppliers/edit?id=" + row._id }}>
    ...
    handleDelete = (props) => {
        ...
        Axios.delete(`${CONSTANTS.SERVER_URL}/api/suppliers/` + props)
            .then(response => {
                this.props.enqueueSnackbar(
                    "Supplier Deleted",
                    {
    ...
    render() {
        ...
        return (
            <>
                <CommonHeader buttonText="Register Supplier" redirectURL={`/admin/suppliers/add`} />
                            ...
                            <GridContainer>
                                <GridItem xs={12} sm={12} md={12}>
                                    <MuiThemeProvider theme={this.getMuiTheme()}>
                                        ...
                                        <MUIDataTable title={""} data={this.generateTableData(this.state.suppliers)} columns={columns} options={options} />
                                    </MuiThemeProvider>
                                </GridItem>
                            </GridContainer>
                        </CardBody>
...
export default withSnackbar(withStyles(customStyles, { name: "SuppliersListView.js" })(SuppliersListView));